...
hrtimer_force_reprogram(struct hrtimer_cpu_base *cpu_base, int skip_equal)
{
	...
	expires_next = __hrtimer_get_next_event(cpu_base, NULL, HRTIMER_ACTIVE_HARD);
...
u64 hrtimer_get_next_event(void)
{
	...
	if (!__hrtimer_hres_active(cpu_base))
		expires = __hrtimer_get_next_event(cpu_base, NULL, HRTIMER_ACTIVE_HARD);
...
u64 hrtimer_next_event_without(const struct hrtimer *exclude)
{
	...
	if (__hrtimer_hres_active(cpu_base))
		expires = __hrtimer_get_next_event(cpu_base, exclude, HRTIMER_ACTIVE_HARD);
