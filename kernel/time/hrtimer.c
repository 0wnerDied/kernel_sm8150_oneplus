 ...
 */
static ktime_t
__hrtimer_get_next_event(struct hrtimer_cpu_base *cpu_base, const struct hrtimer *exclude, unsigned int active_mask)
{
	struct hrtimer_clock_base *base;
	unsigned int active;
...
u64 hrtimer_get_next_event(void)
{
	...
	if (!__hrtimer_hres_active(cpu_base))
		expires = __hrtimer_get_next_event(cpu_base, NULL, HRTIMER_ACTIVE_ALL);
...
u64 hrtimer_next_event_without(const struct hrtimer *exclude)
{
	...
	if (__hrtimer_hres_active(cpu_base))
		expires = __hrtimer_get_next_event(cpu_base, exclude, HRTIMER_ACTIVE_ALL);
